...
_DEFAULT_TOKEN_MIN_PX: dict[str, int] = {
    "--t-projection-hero": 96,
    "--t-projection-title": 72,
    "--t-projection-body": 36,
    "--t-display-1": 96,
    "--t-display-2": 72,
    "--t-h1": 72,
    "--t-h2": 48,
...
CRAFT_REVIEW_ITEMS: tuple[CraftReviewItem, ...] = (
    ...
    CraftReviewItem(
        "PROJECTION_TYPE",
        "index.css 投影可读性：hero ≥96px/800；正文 ≥36px/500；辅助 ≥28px（projection token 或显式 px）",
        "manual",
    ),
    ...
    CraftReviewItem("NO_TINY_TEXT_WALL", "禁止小号字体、大量纯文字（正文 < 36px）", "auto"),
...
def _check_no_tiny_text(css: str, *, ppt: Path | None = None) -> tuple[bool, str]:
    ...
    return True, "body ≥36px / auxiliary ≥28px (SVG/code/window exempt)"
