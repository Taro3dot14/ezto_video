"""Tests for chapter artifact validation (static checks)."""
...
def test_validate_tsx_css_classes_ignores_lx_mot_globals(tmp_path: Path):
    ch = tmp_path / "presentation" / "src" / "chapters" / "hook"
    ch.mkdir(parents=True)
    ch.joinpath("index.tsx").write_text(
        '<div className="lx-cover-body mot-hero-blur mask-reveal in">'
        '<h1 className="lx-hero">Hi</h1></div>',
        encoding="utf-8",
    )
    ch.joinpath("index.css").write_text(".ch-scene { width: 100%; }\n", encoding="utf-8")
