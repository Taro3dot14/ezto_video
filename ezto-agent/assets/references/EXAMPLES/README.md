...
| 例子 | 适用场景 | 文件 |
|---|---|---|
| [`hook-chapter/`](hook-chapter/) | **钩子型开场** —— 多张图片逐张揭示后 hero takeover | → **`lx-solo`** / **`lx-cover`** |
| [`list-reveal/`](list-reveal/) | **列举型** —— 口播说"三件事 / N 个特性"，每项 1 step | → **`lx-grid-3`** + **`GridSlot`** |
...
4. 翻 `chapter.css` 看动画用了哪些 keyframes、token、`infinite` 持续
   微动写在哪
5. 写自己这一章时**保留 anchor 的"形"**，在 scaffold 项目里用 **`lx-*` shell + `GridSlot`** 实现，按本章内容 + 主题换动画

> **2025+ scaffold** 已内置 Layout Shell System（`templates/src/layouts/`）。
> EXAMPLES 仍是结构参考；实现时优先 `01-example` + `LAYOUT-SYSTEM.md`，不必照抄 example 的 class 前缀。
